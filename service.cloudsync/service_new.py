...
MQTT-first real-time sync service for Kodi
"""
...
n        # Favorites polling state
    ...
    def _handle_favorites_message(self, topic: str, payload: dict):
        ...
        try:
            self._log(f"Received favorites message: {topic}", xbmc.LOGDEBUG)
            ...
            self._log(f"Favorites {action} for: {title}")
    ...
    def _main_loop(self):
        """Main service loop with MQTT processing"""
